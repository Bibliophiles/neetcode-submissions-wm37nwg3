class Solution:
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        for pivot_index, pivot in enumerate(nums):
            # Pivot is greater than the value 0, break out of the loop(the goal won't be met)
            if pivot > 0:
                break
            # Checking for duplicate pivot numbers
            if pivot_index > 0 and nums[pivot_index] == nums[pivot_index - 1]:
                continue

            # Find the two complement values to the pivot
